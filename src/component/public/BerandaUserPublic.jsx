...
/**
 * AccordionItem: Mengelola tampilan FAQ.
 * Key: Menggunakan local state 'open' untuk toggle animasi slide down pada jawaban.
 */
...
/**
 * calculateCountdown: Logika matematika untuk menghitung selisih waktu.
 * Key: Menghitung milidetik tersisa dan mengonversinya ke format Hari, Jam, Menit, Detik.
 */
...
//MAIN COMPONENT: BERANDA USER
export default function BerandaUser() {
  //STATES: DATA & FILTERS
  const navigate = useNavigate(); // Untuk navigasi halaman 
  const [events, setEvents] = useState([]);               // Data master dari API
  const [filteredEvents, setFilteredEvents] = useState([]); // Data yang ditampilkan (setelah filter)
  const [categories] = useState(["Semua", "Musik", "Seminar", "Workshop", "Lomba"]);
  const [searchTerm, setSearchTerm] = useState("");         // Kata kunci pencarian
  const [selectedCategory, setSelectedCategory] = useState("Semua"); // Filter kategori terpilih
  
  //STATES: UI CONTROL
  const [openModal, setOpenModal] = useState(false);        // Status buka/tutup Modal Detail
  const [selectedEvent, setSelectedEvent] = useState(null); // Data event yang akan muncul di Modal
  const [countdowns, setCountdowns] = useState({});         // Objek penampung timer real-time

  //LOGIC: FETCHING DATA
  //Key: Mengambil data dari MockAPI dan melakukan 'Normalization' (pembersihan data).
  useEffect(() => {
  ...
  //LOGIC: REAL-TIME TIMER
  //Key: Menggunakan setInterval untuk memperbarui state 'countdowns' setiap 1 detik.
  useEffect(() => {
    ...
    return () => clearInterval(timer); // Cleanup function untuk mencegah memory leak
  }, [events]);

  //LOGIC: FILTERING SYSTEM
  // Key: Melakukan filter ganda (Kategori + Search Term) secara otomatis saat state berubah.
  useEffect(() => {
  ...
  // FUNGSI NAVIGASI KE HALAMAN CHECKOUT (update 2025-01-09)
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans selection:bg-[#39ff14] selection:text-black">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4">
          <div className="relative w-full md:flex-1">
            ...
            <Input
              ...
              className="bg-[#111] border-white/10 rounded-full pl-12 h-12 focus:border-[#39ff14] transition-all"
              value={searchTerm}
      ...
      <main className="max-w-7xl mx-auto py-12 px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredEvents.length > 0 ? (
            (selectedCategory === "Semua" ? filteredEvents.slice(0, 10) : filteredEvents).map((event) => ( //filter pilhan hanya 10 jika semua kategori
              <motion.div
                key={event.id}
                whileHover={{ y: -10 }}
              ...
              >
                <Card className="bg-[#111] border-white/10 overflow-hidden h-full flex flex-col">
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <Badge className={`absolute top-3 left-3 ${event.badge === "Sedang Berlangsung" ? "bg-red-500" : "bg-blue-600"}`}>
                      {event.badge}
                    </Badge>
                  </div>
                  <CardContent className="p-5 flex-grow">
                    ...
                    <div className="flex justify-between items-end mt-auto">
                      <div>
                        <p className="text-white/40 text-[10px] uppercase">Harga</p>
                        <p className="text-[#39ff14] font-bold">{event.price}</p>
                      </div>
                      <div className="text-right">
                        <div className="flex items-center gap-1 text-[10px] text-white/60 mb-1">
                          <Clock size={10} /> {countdowns[event.id] || "Loading..."}
                        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredEvents.length > 0 ? (
            (selectedCategory === "Semua" ? filteredEvents.slice(0, 4) : filteredEvents).map((event) => ( //filter event mendatang
              <motion.div
                key={event.id}
                whileHover={{ y: -10 }}
                onClick={() => handleOpenModal(event)}
              ...
              >
                <Card className="bg-[#111] border-white/10 overflow-hidden h-full flex flex-col">
                  <div className="relative h-48 overflow-hidden">
                    <img src={event.image} alt={event.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    <Badge className="absolute top-3 left-3 bg-white text-black hover:bg-white/90 border-none font-bold">
                      COMING SOON
                    </Badge>
                  </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button className="bg-[#39ff14] hover:bg-[#32cc11] text-black">
                <Mail size={18} />
              </Button>
            </div>
          </div>
      ...
      {/* --- UI: MODAL DETAIL (DIALOG) --- */}
      {/* Key: Dialog ini bersifat kondisional, hanya merender isi jika selectedEvent tidak null. */}
          ...
          <DialogContent className="bg-[#0f0f0f] border-white/10 text-white max-w-xl rounded-2xl overflow-hidden p-0">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-between items-center bg-white/5 p-4 rounded-xl mb-6">
                <div>
                  <p className="text-[10px] text-white/40 uppercase">Harga Tiket</p>
                  <p className="text-[#39ff14] font-bold text-xl">{selectedEvent.price}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-white/40 uppercase">Sisa Tiket</p>
                  <p className="font-bold">{selectedEvent.tickets} Tiket</p>
                </div>
              </div>
              <Button
              onClick={handleGoToCheckout} // Panggil fungsi navigasi ke halaman checkout
              className="w-full bg-[#39ff14] text-black font-bold h-12 rounded-xl hover:scale-[1.02] transition-transform">
                Beli Tiket Sekarang
              </Button>
            </div>
          </DialogContent>
